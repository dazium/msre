...
export function generateEstimatePDF(estimate: any) {
  // Handle both EstimatePDFData and raw estimate objects from DB
  const data: EstimatePDFData = typeof estimate.lineItems === 'object' && !Array.isArray(estimate.lineItems)
    ? estimate
    : {
        estimateNumber: estimate.estimateNumber || 'EST-001',
        customerName: estimate.customerName || 'Customer',
        projectTitle: estimate.projectTitle || estimate.title || 'Project',
        lineItems: estimate.lineItems || [],
        subtotal: estimate.subtotal || 0,
        total: estimate.total || 0,
        createdDate: estimate.createdDate || new Date(),
        validUntil: estimate.validUntil,
        companyName: estimate.companyName || 'Roofing CRM',
...
export function generateInvoicePDF(invoice: any) {
  // Handle both InvoicePDFData and raw invoice objects from DB
  const data: InvoicePDFData = {
    invoiceNumber: invoice.invoiceNumber || 'INV-001',
    customerName: invoice.customerName || 'Customer',
    projectTitle: invoice.projectTitle || invoice.title || 'Project',
    ...
    companyName: invoice.companyName || 'Roofing CRM',
  ...
  doc.setTextColor(60, 60, 60);
  const detailsY = margin + 35;
  doc.text(`Invoice #: ${data.invoiceNumber}`, margin, detailsY);
  doc.text(`Date: ${new Date(data.createdDate).toLocaleDateString()}`, margin, detailsY + 7);
  if (data.dueDate) {
    doc.text(`Due Date: ${new Date(data.dueDate).toLocaleDateString()}`, margin, detailsY + 14);
  }
  ...
  doc.text(`Subtotal: $${data.subtotal.toFixed(2)}`, pageWidth - margin - 50, finalY);
  if (data.tax > 0) {
    doc.text(`Tax: $${data.tax.toFixed(2)}`, pageWidth - margin - 50, finalY + 7);
  }

  doc.setFontSize(12);
  doc.setTextColor(10, 22, 40);
  doc.setFont('helvetica', 'bold');
  doc.text(`Total: $${data.total.toFixed(2)}`, pageWidth - margin - 50, finalY + (data.tax > 0 ? 14 : 7));

  // Payment Terms
  if (data.paymentTerms) {
    doc.setFontSize(9);
    doc.setTextColor(100, 100, 100);
    doc.text(`Payment Terms: ${data.paymentTerms}`, margin, finalY + 20);
  }
  ...
  // Save PDF
  const filename = data.invoiceNumber ? `invoice-${data.invoiceNumber}.pdf` : 'invoice.pdf';
  doc.save(filename);
}
